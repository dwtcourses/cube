...
<div class="vue-monthly-picker">
	<div class="month-picker-wrapper" :class="{ 'active visible':showMenu }">
		<div class="month-year-label picker" type="text" autocomplete="off" tabindex="0" @click="openMenu">
			<div @click="openMenu" class="month-year-display" :disabled="disabled" :class="[inputClass, {'placeholder': !value}]">
				{{ displayText }}
			</div>
		</div>
		<div class="text"></div>
			...
			<div class="picker" style="text-align: center">
				...
				<div class="flexbox monthItem">
					<template v-for="(month, idx) in monthLabels">
						<div class="item active" :class="{'selected': isCurrentSelected(year, idx)}" v-if="isActive(idx)" @click="selectMonth(idx)" :key="idx">{{month}}
						</div>
						<div v-else :class="{'selected': isCurrentSelected(year, idx)}" :key="idx" class="item deactive">
							{{ month }}
						</div>
					</template>
				</div>
...
export default {
	...
	computed: {
		...
		internalMomentValue() {
			...
			console.log(dayjs(yrMonth).format(`YYYY${this.spliter}MM`));
			return dayjs(yrMonth).format(`YYYY${this.spliter}MM`);
		}
	},
	methods: {
		init() {
