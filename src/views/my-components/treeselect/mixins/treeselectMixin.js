...
export default {
	...
	watch: {
		...
		value(newValue) {
			...
			if (typeof(newValue) == 'object') {
				...
				_newValue = [newValue.id];
			} else {
				_newValue = (!newValue && newValue !== 0) ? [] :
					this.multiple ?
					newValue.slice() : [newValue];
			}
	...
	methods: {
		...
		getNode(nodeId) {
			warning(
				() => nodeId != null || nodeId != '' || nodeId != undefined,
		...
		initializeValue() {
			if (this.multiple) {
				...
				this.internalValue = this.value != null && this.value != '' && this.value != undefined ? [this.value] : []
			}
		},
		...
		normalize(parentNode, nodes) {
			let normalizedOptions = nodes.map((node, index) => {
				...
				const lowerCasedLabel = name.toLowerCase() // used for option filtering
		...
		clear() {
			if (this.hasValue) {
				this.internalValue = this.multiple ? this.internalValue.filter(nodeId => this.getNode(nodeId).isDisabled) : [];
				this.buildSelectedNodeMap();
				this.buildNodeCheckedStateMap();
			}
		},
